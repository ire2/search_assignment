...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                nextPos = (nextx, nexty)
                if not nextPos in self.corners:
                    ...
                    for i, corner in enumerate(self.corners):
                        if corner == nextPos:
                            ...
                            break
                    successors.append(
                        ((nextPos, tuple(visited)), action, 1))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    current_position, current_corners = state
    unvisited = [corners[i]
                 for i, visited in enumerate(current_corners) if not visited]
    h = 0
    if not unvisited:
        return h
    while unvisited:
        distances = [(0.75 * util.manhattanDistance(current_position, corner) +
                      0.25 * euclideanHeuristic(current_position, corner), corner)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    distance = [mazeDistance(
        current_position, food, problem.startingGameState) for food in food_coordinates]
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        # are these variables not redundant considering were using any search problem?
        ...
        "*** BEGIN YOUR CODE HERE ***"
        # it does not return the shortest path in the entire maze because
        # it is a greedy algorithm  best decision at given x point
        # not for the entire maze, if we used appropriate heurestics it would optimize the
        # paths in the maze to encompass the entire goals (ie, eating all the food in shortest path)
        # not necessarily choosing the closest food to eat
